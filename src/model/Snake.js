...
Snake.prototype = {
  ...
  addCube: function () {
    this.snakeArray.push(
      this.createBlock(
        this.snakeHead.prev_position.x,
        this.snakeHead.prev_position.y,
        this.snakeHead.prev_position.z
      )
    );
  },
  ...
  isHit: function (blk1, blk2) {
    if (
      blk1.position.x == blk2.position.x &&
      blk1.position.z == blk2.position.z
    ) {
      return true;
    } else {
      return false;
  ...
  updateBodyPos: function () {
    _.drop(this.snakeArray).forEach((block, index) => {
      if (index === 0) {
        ...
        block.prev_position = block.position.clone();
        block.position.set(
          this.prevblock_prevpos.x,
          this.prevblock_prevpos.y,
          this.prevblock_prevpos.z
        );
        this.prevblock_prevpos = block.prev_position.clone();
      }
    });
